...
alias ros2-build='f(){ colcon build --symlink-install "$@" && . install/setup.bash; unset -f f; }; f'
alias ros2-build-clear='f(){ rm -rf install/ build/ && colcon build "$@" && . install/setup.bash; unset -f f; }; f'
alias ros2-build-debug='f(){ colcon build --symlink-install --cmake-args -DCMAKE_BUILD_TYPE=Debug "$@" && . install/setup.bash; unset -f f; }; f'
